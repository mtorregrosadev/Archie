...
archie config     # menú interactiu per configurar opcions
...
A continuació es mostra un diagrama ASCII de com interactuen les diferents parts de l'Archie:
...
```text
                  ...
                  +--------------------------------+
                                 ^
                                 | (Sensors, CPU, RAM, processsos)
  +------------------+           v             +------------------+
  |    brain.py      | <--- (Mètriques) -----> |    monitor.py    |
  | (Intel·ligència, |                         | (Motor de checks)|
  | Hàbits, Ghosts)  |                         | Llegeix el YAML  |
  +------------------+                         +------------------+
          ^                                            |
          | (Feedback: Accept/Dismiss)                 | (Alertes)
          v                                            v
  +-------------------------------------------------------------+
  |                         archie.py                           |
  |                       (Aplicació / CLI)                     |
  +-------------------------------------------------------------+
          |                                            |
   (Mode Daemon)                                (Comandes CLI)
          |                                            |
          v                                            v
  +------------------+                         +------------------+
  |  GTK4 Layer UI   |                         | Sortida Terminal |
  | (Mascota visual) |                         | (status, brain,  |
  +------------------+                         |  fix, config...) |
```
